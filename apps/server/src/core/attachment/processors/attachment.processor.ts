...
export class AttachmentProcessor extends WorkerHost implements OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly attachmentIndexService: AttachmentIndexService,
  ) {
    super();
  }
  ...
  async process(job: Job<any, void>): Promise<void> {
    try {
      ...
      if (job.name === QueueJob.ATTACHMENT_INDEX_CONTENT) {
        await this.attachmentIndexService.indexAttachment(job.data.attachmentId);
      } else if (job.name === QueueJob.ATTACHMENT_INDEXING) {
        await this.attachmentIndexService.indexAttachments(job.data.workspaceId);
      }
    } catch (err) {
      throw err;
    }
